...
private[spark] object SamplingUtils {
  ...
  /**
   * Reservoir stratified sampling implementation that also returns the weights.
   *
   * @param input input size
   * @param sampleSize reservoir sizes
  ...
  : mutable.Map[K, (Array[(K, V)], Long)] = {

    var reservoirs: mutable.Map[K, (Array[(K, V)], Long)] = new mutable.HashMap[K, (Array[(K, V)], Long)]()
    val counts:mutable.Map[K, Long] = new mutable.HashMap[K, Long]()
    var weight:Long = 0
    ...
    while (input.hasNext) {
      ...
      if (!reservoirs.contains(key)) {
        reservoirs += (key -> (new Array[(K, V)](sampleSize), weight))
        counts += (key -> 0)
      }

      // Put the first k elements in the reservoir.
      if (counts(key) < sampleSize) {
        weight = 1
        reservoirs(key)._1(counts(key).toInt) = item
        counts(key) += 1
      } else {
        //if input size > sampleSize, replacing item in reservoir
        weight = counts(key)/sampleSize
        val replacementIndex = (rand.nextDouble() * counts(key)).toLong
        if (replacementIndex < sampleSize) {
          reservoirs(key)._1(replacementIndex.toInt) = item
        }
        counts(key) += 1
      }
      reservoirs(key) = reservoirs(key).copy(_2 = weight)
    }
    reservoirs
  }
