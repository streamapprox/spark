...
class PairRDDFunctions[K, V](self: RDD[(K, V)])
  ...
  /**
   * Return a subset of this RDD sampled by key (via reservoir stratified sampling).
   *
   ...
   */
  def reservoirStratifiedSample(sampleSize: Int,
                                seed: Long = Utils.random.nextLong): RDD[(K, V)] = self.withScope {

    require(sampleSize >= 0.0, "Negative sampling rates.")

    val samplingFunc = StratifiedSamplingUtils
      .getReservoirSamplingFunction(self, sampleSize, seed)
